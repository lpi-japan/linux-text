# Linuxのインストールと設定
第3章では、VirtualBoxで作成した仮想マシンにAlmaLinuxをインストールします。OSインストール後、初期設定とネットワーク接続の確認を行います。
...
「127.0.0.1/8」はローカルループバックのIPアドレスです。前の実習でWebブラウザでアクセスするアドレスを「localhost」と指定しましたが、IPアドレスはこちらを指定していたことになります。

「10.0.2.15/24」はNATに設定したアダプターのIPアドレスです。インターネットなどホストOSの外側にあるネットワークに接続するために使用されます。
...
Windowsのコマンドプロンプトを実行します。タスクバーの検索ウインドウに「cmd」と入力し、候補として出てきたコマンドプロンプトを実行します。
